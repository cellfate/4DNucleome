%% logarithmic gains (two partial derivatives) for kep Figure S14A-C
clear;
K = 10.^linspace(-3,1,500); 
% rates = 0.003:0.001:0.6;% ini
rates = 0.010:0.001:0.2;% rec
%  rates = 0.010:0.001:0.6;% rel
dlogMBSdlogKEP = zeros(size(rates,2),size(K,2));
dlogBFdlogKEP = zeros(size(rates,2),size(K,2));
...
for r_idx = 1:1:size(rates,2)
    for idx = 1:1:size(K,2)
        ...
        input_options.k_on1_max         = rates(r_idx); % burst initiation rate
       ...
       %%
        dFdKEP = 1./sqrt(2*pi).*(d_05./sqrt(params.diffusion_const.*params.friction_coef)).^3.*...
            sqrt((params.spring_const(1,1)./(params.promoter_index - params.enhancer_index)...
            +params.attraction_coef)).*exp(-0.5.*(d_05./sqrt(params.diffusion_const.*...
            params.friction_coef)).^2.*((params.spring_const(1,1)./(params.promoter_index...
            - params.enhancer_index)+params.attraction_coef)));
        ...
        dlogMBSdlogKEP(r_idx,idx) = params.attraction_coef.*dFdKEP.*(UpperBS./DownerBS);
        ...
        dlogMCTdlogKEP = params.attraction_coef.*dFdKEP.*(UpperCT./DownerCT);
        dlogBFdlogKEP(r_idx,idx) = -dlogMCTdlogKEP; 
    end
end
...
figure
S1 = surf(log10(K),rates,dlogMBSdlogKEP);
S1.EdgeColor = 'none';
hold on
S2 = surf(log10(K),rates,dlogBFdlogKEP,'FaceAlpha',0.8);
S2.EdgeColor = 'none';
box on
