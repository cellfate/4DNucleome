...
enhancer_index = 51:1:99;
promoter_index = 49:-1:1;
k = linspace(log10(0.0001),log10(0.01),50);
IBSDS = zeros(size(enhancer_index,2),size(k,2));
ICTDS = zeros(size(enhancer_index,2),size(k,2));
IOFFDS= zeros(size(enhancer_index,2),size(k,2));
IONDS = zeros(size(enhancer_index,2),size(k,2));
for jdx = 1:1:50
    for idx = 1:1:size(enhancer_index,2)
        input_options.EP_flag           = true;
        input_options.attraction_coef   = 0.1; % K, E-P communication coefficient
        input_options.enhancer_index    = enhancer_index(idx); % enhancer index number
        input_options.promoter_index    = promoter_index(idx); % promoter index number
        
        % Downstream: Transcriptional bursting
        input_options.k_on1_max         = 0.020; % burst initiation rate
        input_options.k_on1             = 10^(k(jdx)); % burst initiation rate
        ...
        input_options.k_release         = 0.010; % burst polymerase pause release rate
