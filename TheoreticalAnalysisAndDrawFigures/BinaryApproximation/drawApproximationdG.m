%% approximation and power-law for dG Figure S14IJ
dG = linspace(0,2,500);
output_size = size(dG,2);
...
for i = 1: output_size
    params.EPGenomicDist = 10^(dG(i));
    params.attraction_coef = 0.1;
    ...
    maxvelocity = (params.spring_const(1,1)/(abs(params.EPGenomicDist))+...
        params.attraction_coef)*d_EP(find(cumsum(P)>99,1, 'first'))/params.friction_coef;
...
bs = log10(BS_max) + dlogBS_dlogk.*((abs(dG)));
bf = -log10(TOFF_max+TON_max) + dlogBF_dlogk.*((abs(dG)));
%% figure
%% burst size
figure1 = figure;
set(figure1,'position',[300 400 280 190],'Name','bs');
plot((dG),log10(burst_size_theor));
hold on
plot((dG), log10(burst_size_theor_a));
plot((dG), bs);
box on
axis([-0.05 2.05 -0.1 0.4])
...
hold on
plot((dG),log10(burst_frequency_theor));
plot((dG),log10(burst_frequency_theor_a));
plot((dG), bf);
axis([-0.05 2.05 -3.2 -2.5])
box on
set(gca,'TickLength',[0.02,0.025]);
